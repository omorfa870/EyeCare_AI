...
export default function ManageDoctorsPage() {
  ...
  }, [searchQuery, doctors, pendingDoctors, activeTab]);
  ...
  const renderPendingCard = (doctor: Doctor) => {
    const user = typeof doctor.user === "object" ? doctor.user : null;
    if (!user) return null;

    return (
      <motion.div variants={fadeIn} key={doctor._id}>
        <Card className="group hover:shadow-xl transition-all duration-300 border-border/50 bg-card/60 backdrop-blur-md overflow-hidden rounded-3xl relative border-l-4 border-l-amber-500">
          <CardContent className="p-6">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              <div className="flex items-center gap-5">
                <Avatar className="h-20 w-20 border-4 border-background shadow-lg rounded-2xl">
                  <AvatarFallback className="text-2xl font-black rounded-2xl bg-amber-500/10 text-amber-500">
                ...
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <Badge className="px-2 py-0.5 text-[10px] uppercase font-black tracking-widest border bg-amber-500/10 text-amber-600 border-amber-500/20">
                  ...
                  <div className="mt-1 text-xs text-muted-foreground">
                    Reg: <span className="text-foreground font-semibold">{doctor.registrationNumber}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                <Button
                  ...
                  className="rounded-xl font-bold h-10 px-5 bg-emerald-600 hover:bg-emerald-700 text-white"
                >
  ...
  return (
    <div className="flex-1 w-full bg-background flex flex-col relative overflow-hidden">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <div className="flex items-center justify-between mb-8 overflow-x-auto pb-2">
               <TabsList className="bg-muted/50 p-1.5 rounded-2xl h-auto border border-border/50">
                ...
                <TabsTrigger value="pending" className="rounded-xl px-6 py-2.5 data-[state=active]:bg-background data-[state=active]:shadow-md font-bold">
                  Pending{" "}
                  {pendingDoctors.length > 0 && (
                    <Badge className="ml-2 bg-amber-500 text-white border-none">{pendingDoctors.length}</Badge>
                  )}
                </TabsTrigger>
                    ...
                    <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="grid gap-6">
                      {pendingDoctors.length === 0 ? (
                        <Card className="border-dashed border-2 border-border/40 bg-transparent py-20 text-center rounded-[2.5rem]">
                          ...
                          <p className="text-xl font-bold text-muted-foreground">No pending approval requests.</p>
                        </Card>
                      ) : pendingDoctors.map(renderPendingCard)}
                    </motion.div>
                 </TabsContent>
      ...
      <AlertDialog open={!!deleteDialog} onOpenChange={() => setDeleteDialog(null)}>
        <AlertDialogContent className="rounded-4xl border-border/50 bg-card/95 backdrop-blur-xl">
          <AlertDialogHeader>
